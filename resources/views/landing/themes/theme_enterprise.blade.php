...
<div x-data="{
    ...
    {{-- ========================================================================= --}}
    <section id="pricing" class="landing-sec-pricing landing-dark-section py-20 sm:py-28 w-full border-b border-slate-200 dark:border-slate-800 transition-colors duration-300">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="text-center mb-12">
            <span class="inline-flex items-center gap-1.5 px-3.5 py-1 rounded-full bg-emerald-50 dark:bg-emerald-500/10 border border-emerald-200 dark:border-emerald-500/20 text-emerald-700 dark:text-emerald-400 text-xs font-black uppercase tracking-wider mb-3">
                {{ $branding->getSectionBadge('pricing', __('Transparent SaaS Investment')) }}
            </span>
            <h2 class="text-3xl sm:text-5xl font-black text-slate-900 dark:text-white tracking-tight">
                {{ $branding->getSectionTitle('pricing', __('Enterprise Capacity for Every Business Scale')) }}
            </h2>
            <p class="mt-4 text-slate-600 dark:text-slate-400 max-w-2xl mx-auto text-sm sm:text-base">
                {{ $branding->getSectionSubtitle('pricing', __('Instant workspace activation with zero setup fees. Direct checkout integration.')) }}
            </p>
        </div>

        <!-- Monthly / Annual Toggle -->
        <div class="flex items-center justify-center gap-3 mb-14">
            <span class="text-xs sm:text-sm font-bold" :class="!annualBilling ? 'text-slate-900 dark:text-white' : 'text-slate-500 dark:text-slate-400'">{{ __('Monthly Billing') }}</span>
            <button type="button" @click="annualBilling = !annualBilling"
                    :class="annualBilling ? 'bg-emerald-500' : 'bg-slate-200 dark:bg-slate-800'"
                    class="relative w-14 h-8 rounded-full transition-colors p-1 cursor-pointer">
                <span class="block w-6 h-6 rounded-full bg-white dark:bg-slate-950 shadow transition-transform"
                      :class="annualBilling ? 'translate-x-6' : 'translate-x-0'"></span>
            </button>
            <span class="text-xs sm:text-sm font-bold flex items-center gap-2" :class="annualBilling ? 'text-slate-900 dark:text-white' : 'text-slate-500 dark:text-slate-400'">
                {{ __('Annual Billing') }}
                <span class="px-2.5 py-0.5 rounded-full bg-emerald-500/20 text-emerald-700 dark:text-emerald-400 text-[10px] font-black uppercase tracking-wider border border-emerald-500/30">{{ __('Save 20%') }}</span>
            </span>
        </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 items-stretch">
                @foreach ($plans as $plan)
                    ...
                    <div class="rounded-3xl p-8 flex flex-col justify-between transition-all duration-300 relative
                        {{ $isPopular 
                            ? 'bg-white dark:bg-gradient-to-b dark:from-slate-900 dark:to-slate-950 border-2 border-emerald-500 shadow-2xl shadow-emerald-500/10 scale-[1.02]' 
                            : 'bg-white dark:bg-slate-900/70 border border-slate-200 dark:border-slate-800 hover:border-slate-300 dark:hover:border-slate-700 shadow-sm' }}">
                        ...
                        <div>
                            ...
                            <div class="mt-4 flex items-baseline gap-1.5">
                                <span x-show="!annualBilling" class="text-4xl font-black text-slate-900 dark:text-white">{{ $plan->currency }}{{ number_format($price, 2) }}</span>
                                <span x-show="!annualBilling" class="text-xs text-slate-500 dark:text-slate-400 font-medium">/{{ $plan->billing_cycle }}</span>
                                <span x-show="annualBilling" x-cloak class="text-4xl font-black text-slate-900 dark:text-white">{{ $plan->currency }}{{ number_format($annualPrice, 2) }}</span>
                                <span x-show="annualBilling" x-cloak class="text-xs text-slate-500 dark:text-slate-400 font-medium">/{{ __('year') }}</span>
                            </div>

                            <ul class="mt-8 space-y-3 text-xs text-slate-700 dark:text-slate-300">
