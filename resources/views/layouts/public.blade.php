@php
    $publicReferenceLanding = (bool) ($referenceLanding ?? false);
    ...
    foreach ($publicBranding->landingSectionOrder() as $landingSectionIndex => $landingSectionKey) {
        ...
        $selector = match ($landingSectionKey) {
            'hero' => 'showcase',
            'downloads' => 'download',
...
<head>
    ...
    @php
        $brandEmerald = $publicBranding->landing_primary_color;
        if (empty($brandEmerald) || in_array(strtoupper(trim($brandEmerald)), ['#FFFFFF', '#FEFEFE', '#FDFDFD', '#000000'], true)) {
            $brandEmerald = '#10b981';
        }
        $brandLime = $publicBranding->landing_accent_color;
        if (empty($brandLime) || in_array(strtoupper(trim($brandLime)), ['#FFFFFF', '#FEFEFE', '#FDFDFD', '#000000'], true)) {
            $brandLime = '#d7f24e';
        }
        $brandTeal = $publicBranding->primary_color;
        if (empty($brandTeal) || in_array(strtoupper(trim($brandTeal)), ['#FFFFFF', '#FEFEFE', '#FDFDFD', '#000000'], true)) {
            $brandTeal = '#0c5966';
        }
    @endphp
    ...
    <style>
        :root {
            --color-brand-emerald: {{ $brandEmerald }};
            --color-brand-lime: {{ $brandLime }};
...
<body class="public-site {{ $publicReferenceLanding ? 'reference-site' : '' }} bg-white text-slate-900 dark:bg-slate-950 dark:text-slate-100 font-sans antialiased min-h-screen selection:bg-brand-lime selection:text-slate-900">

    @if ($publicReferenceLanding)
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
            ...
            <div class="mt-12 pt-8 border-t border-slate-200 dark:border-white/10 flex flex-col md:flex-row items-center justify-between gap-4 text-xs text-slate-500">
                ...
                <div class="flex items-center gap-5 text-slate-500 dark:text-slate-400">
                    @if ($publicReferenceLanding)
                        <button type="button" x-on:click="dark = !dark" class="reference-theme-toggle" title="{{ __('Toggle Theme') }}" :aria-pressed="dark.toString()">
                            <x-landing.icon name="bolt" width="16" height="16" />
                            <span x-show="!dark">{{ __('Dark mode') }}</span>
                            <span x-show="dark" x-cloak>{{ __('Light mode') }}</span>
                        </button>
                    @endif
