<div class="w-full space-y-6 max-w-7xl mx-auto" x-data="{
    ...
    <!-- Top Notification Flash Banner -->
    @if (session('status'))
        <div class="p-4 rounded-2xl bg-emerald-50 dark:bg-emerald-950/40 text-emerald-700 dark:text-emerald-300 text-xs sm:text-sm font-bold flex items-center justify-between gap-3 border border-emerald-200 dark:border-emerald-800 shadow-sm animate-in fade-in">
            ...
            <a href="{{ url('/') }}" target="_blank" class="text-xs font-black text-emerald-800 dark:text-emerald-200 underline hover:no-underline">
                {{ __('View Live Site') }} ↗
            </a>
        </div>
    @endif
    ...
    <div class="bg-white dark:bg-slate-900 rounded-3xl p-6 sm:p-8 shadow-[0_4px_25px_rgb(0,0,0,0.03)] border border-slate-100 dark:border-slate-800 flex flex-col md:flex-row md:items-center justify-between gap-6 relative overflow-hidden">
        <div class="space-y-2 relative z-10">
            <div class="flex items-center gap-3">
                <div class="w-11 h-11 rounded-2xl bg-gradient-to-br from-indigo-500 via-purple-500 to-pink-500 text-white flex items-center justify-center text-xl font-black shadow-md shadow-indigo-500/20">
                    🌐
                </div>
                <div>
                    <h1 class="text-xl sm:text-2xl font-black text-slate-900 dark:text-white tracking-tight leading-tight flex items-center gap-2.5">
                        <span>{{ __('Landing Page & Menus Setup') }}</span>
                        <span class="px-2.5 py-0.5 rounded-full text-[10px] font-black uppercase tracking-wider bg-indigo-100 text-indigo-800 dark:bg-indigo-950 dark:text-indigo-300">
                            {{ __('Platform Control') }}
                        </span>
                    </h1>
                    <p class="text-xs sm:text-sm text-slate-500 dark:text-slate-400 font-medium">
                        {{ __('Configure your homepage display (Modular SaaS Theme vs Static CMS Permalink), manage navigation menus, and customize white-label identity.') }}
                    </p>
                </div>
            </div>
        </div>
        ...
        <div class="flex items-center gap-3 relative z-10 shrink-0">
            <a href="{{ url('/') }}" target="_blank" class="px-4 py-2.5 rounded-2xl text-xs font-bold bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 transition inline-flex items-center gap-1.5 cursor-pointer shadow-xs">
                <span>👁️</span>
            ...
            <button wire:click="save" type="button" wire:loading.attr="disabled"
                    class="px-6 py-2.5 rounded-2xl text-xs sm:text-sm font-black bg-indigo-600 hover:bg-indigo-700 text-white shadow-xl shadow-indigo-500/25 active:scale-95 transition flex items-center gap-2 cursor-pointer">
    ...
    <!-- Clean 3-Tab Rail -->
    <div class="bg-white dark:bg-slate-900 rounded-2xl p-1.5 shadow-sm border border-slate-100 dark:border-slate-800 flex items-center gap-1">
        <button type="button" @click="setTab('landing')"
                :class="activeTab === 'landing' ? 'bg-indigo-600 text-white font-black shadow-sm' : 'text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 font-bold'"
                class="flex-1 py-2.5 px-4 rounded-xl text-xs sm:text-sm transition flex items-center justify-center gap-2 cursor-pointer">
            <span>🌐</span>
            <span>{{ __('1. Homepage & Permalink Setup') }}</span>
        </button>

        <button type="button" @click="setTab('menus')"
                :class="activeTab === 'menus' ? 'bg-indigo-600 text-white font-black shadow-sm' : 'text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 font-bold'"
                class="flex-1 py-2.5 px-4 rounded-xl text-xs sm:text-sm transition flex items-center justify-center gap-2 cursor-pointer">
            <span>🧭</span>
        ...
        <button type="button" @click="setTab('identity')"
                :class="activeTab === 'identity' ? 'bg-indigo-600 text-white font-black shadow-sm' : 'text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 font-bold'"
                class="flex-1 py-2.5 px-4 rounded-xl text-xs sm:text-sm transition flex items-center justify-center gap-2 cursor-pointer">
            <span>🎨</span>
    ...
    <!-- ===================================================================== -->
    <!-- TAB 1: HOMEPAGE & PERMALINK SETUP -->
    <!-- ===================================================================== -->
    <div x-show="activeTab === 'landing'" x-cloak class="space-y-6">

        <!-- Master Switch: Landing Page Enabled -->
        <div class="bg-white dark:bg-slate-900 rounded-3xl p-6 sm:p-8 shadow-[0_4px_25px_rgb(0,0,0,0.03)] border border-slate-100 dark:border-slate-800 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div class="space-y-1">
                <div class="flex items-center gap-2.5">
                    ...
                    @if ($landingPageEnabled)
                        <span class="px-2.5 py-0.5 rounded-full text-[10px] font-black uppercase tracking-wider bg-emerald-100 text-emerald-800 dark:bg-emerald-950 dark:text-emerald-300">
                            {{ __('Online at /') }}
                        </span>
                    @else
                        <span class="px-2.5 py-0.5 rounded-full text-[10px] font-black uppercase tracking-wider bg-slate-100 text-slate-700 dark:bg-slate-800 dark:text-slate-300">
                            {{ __('Redirect to Login') }}
                        </span>
                    @endif
                </div>
                <p class="text-xs sm:text-sm text-slate-500 dark:text-slate-400 font-medium max-w-2xl">
        ...
        <!-- Homepage Mode Picker: Modular SaaS vs Static CMS Permalink -->
        <div class="bg-white dark:bg-slate-900 rounded-3xl p-6 sm:p-8 shadow-[0_4px_25px_rgb(0,0,0,0.03)] border border-slate-100 dark:border-slate-800 space-y-6">
            ...
            <!-- ========================================================= -->
            <!-- CONFIGURATION FOR: STATIC CMS PAGE (PERMALINK SETUP)      -->
            <!-- ========================================================= -->
            @if ($homepageMode === 'static_page')
                <div class="pt-5 border-t border-slate-100 dark:border-slate-800 space-y-5 animate-in fade-in">
                    ...
                    @if ($selectedLandingPage)
                        <!-- Selected Page Card & Live Permalinks -->
            ...
            <!-- ========================================================= -->
            <!-- CONFIGURATION FOR: MODULAR SAAS THEME                     -->
            <!-- ========================================================= -->
            @if ($homepageMode === 'modular')
                <div class="pt-5 border-t border-slate-100 dark:border-slate-800 space-y-6 animate-in fade-in">
                    ...
                    <div class="space-y-3">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                            ...
                            <label class="p-4 rounded-2xl border-2 flex items-center justify-between gap-3 cursor-pointer transition {{ $landingTheme === 'theme_modern' ? 'border-emerald-500 bg-emerald-50/40 dark:bg-emerald-950/20' : 'border-slate-200 dark:border-slate-700 hover:border-slate-300' }}">
                                <div class="flex items-center gap-3">
    ...
    <!-- ===================================================================== -->
    <!-- TAB 2: NAVIGATION MENU ITEMS SETUP                                    -->
    <!-- ===================================================================== -->
    <div x-show="activeTab === 'menus'" x-cloak class="space-y-6">
        <div class="bg-white dark:bg-slate-900 rounded-3xl p-6 sm:p-8 shadow-[0_4px_25px_rgb(0,0,0,0.03)] border border-slate-100 dark:border-slate-800 space-y-6">
            ...
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-100 dark:border-slate-800 pb-4">
                ...
                <div class="flex items-center gap-1.5 p-1 rounded-xl bg-slate-100 dark:bg-slate-800 shrink-0">
                    <button type="button" wire:click="setMenuLocation('header')"
                            class="px-3 py-1.5 rounded-lg text-xs font-bold transition cursor-pointer {{ $menuLocation === 'header' ? 'bg-white dark:bg-slate-900 text-indigo-600 dark:text-indigo-400 shadow-sm' : 'text-slate-600 dark:text-slate-400 hover:text-slate-900' }}">
                        {{ __('Header Bar') }}
                    </button>
                    <button type="button" wire:click="setMenuLocation('footer_col_1')"
                            class="px-3 py-1.5 rounded-lg text-xs font-bold transition cursor-pointer {{ $menuLocation === 'footer_col_1' ? 'bg-white dark:bg-slate-900 text-indigo-600 dark:text-indigo-400 shadow-sm' : 'text-slate-600 dark:text-slate-400 hover:text-slate-900' }}">
                        {{ __('Footer Col 1') }}
                    </button>
                    <button type="button" wire:click="setMenuLocation('footer_col_2')"
                            class="px-3 py-1.5 rounded-lg text-xs font-bold transition cursor-pointer {{ $menuLocation === 'footer_col_2' ? 'bg-white dark:bg-slate-900 text-indigo-600 dark:text-indigo-400 shadow-sm' : 'text-slate-600 dark:text-slate-400 hover:text-slate-900' }}">
                        {{ __('Footer Col 2') }}
                    </button>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
                ...
                <div class="p-4 rounded-2xl bg-slate-50 dark:bg-slate-800/40 border border-slate-200 dark:border-slate-700 space-y-3">
                    ...
                    <div class="flex items-center gap-2">
                        <select wire:model="newMenuPageId" class="flex-1 px-3 py-1.5 rounded-xl border border-slate-200 dark:border-slate-700 dark:bg-slate-800 text-xs font-medium">
                        ...
                        <button type="button" wire:click="addPageMenuLink"
                                class="px-3.5 py-1.5 rounded-xl text-xs font-bold bg-indigo-600 hover:bg-indigo-700 text-white transition shrink-0 cursor-pointer shadow-sm">
                            + {{ __('Add Page') }}
                        </button>
                    </div>
                </div>
            ...
            <div class="space-y-3 pt-2">
                <div class="flex items-center justify-between">
                    ...
                    <span class="text-[11px] text-slate-400">{{ __('Drag or use arrows to reorder') }}</span>
                </div>
    ...
    <!-- ===================================================================== -->
    <!-- TAB 3: BRAND IDENTITY & WHITE-LABEL                                   -->
    <!-- ===================================================================== -->
    <div x-show="activeTab === 'identity'" x-cloak class="space-y-6">
        <div class="bg-white dark:bg-slate-900 rounded-3xl p-6 sm:p-8 shadow-[0_4px_25px_rgb(0,0,0,0.03)] border border-slate-100 dark:border-slate-800 space-y-6">
            
            <div class="border-b border-slate-100 dark:border-slate-800 pb-4">
                <h3 class="text-base sm:text-lg font-black text-slate-900 dark:text-white flex items-center gap-2">
                    <span>🎨</span> {{ __('Platform Brand Identity & White-label') }}
                </h3>
                <p class="text-xs text-slate-400 mt-1">
    ...
    <div class="sticky bottom-4 z-40 bg-white/95 dark:bg-slate-900/95 backdrop-blur-xl border border-slate-200/80 dark:border-slate-800 rounded-3xl p-4 sm:p-5 shadow-2xl flex items-center justify-between gap-4">
        <div class="flex items-center gap-3 text-xs text-slate-500 dark:text-slate-400">
            <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-emerald-50 dark:bg-emerald-950/60 text-emerald-700 dark:text-emerald-300 font-bold text-[11px]">
                <span class="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse"></span>
                <span>{{ __('Live Cache Auto-Purged on Save') }}</span>
            </span>
            <span class="hidden sm:inline">&middot;</span>
            <span class="hidden sm:inline">
                {{ __('Mode') }}: <strong class="text-slate-700 dark:text-slate-200">{{ $homepageMode === 'static_page' ? __('Static CMS Permalink') : __('Modular SaaS Theme') }}</strong>
            </span>
        </div>

        <div class="flex items-center gap-3">
            <a href="{{ url('/') }}" target="_blank" class="px-4 py-2.5 rounded-2xl text-xs font-bold bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 transition inline-flex items-center gap-1 cursor-pointer">
                <span>👁️ {{ __('Preview') }}</span>
                <span>↗</span>
            </a>

            <button wire:click="save" type="button" wire:loading.attr="disabled"
                    class="px-8 py-3 rounded-2xl text-xs sm:text-sm font-black bg-indigo-600 hover:bg-indigo-700 text-white shadow-xl shadow-indigo-500/25 active:scale-95 transition flex items-center gap-2 cursor-pointer">
                <span wire:loading.remove wire:target="save">💾 {{ __('Save All Changes') }}</span>
                <span wire:loading wire:target="save" class="flex items-center gap-1.5">
                    <span class="animate-spin">⏳</span>
                    <span>{{ __('Saving...') }}</span>
