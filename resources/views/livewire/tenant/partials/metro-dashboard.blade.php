...
<section class="metro-dashboard rounded-[2rem] border border-slate-200 dark:border-[#233d59] bg-slate-50 dark:bg-[#07172b] p-4 sm:p-6 lg:p-7 text-slate-900 dark:text-slate-100 shadow-xl space-y-5" aria-label="{{ __('Store dashboard') }}">
    ...
    <div class="grid grid-cols-2 gap-3 lg:grid-cols-4">
        @foreach ([
            ['label' => __('Total Sales'), 'value' => $currencySymbol.number_format($weekTotal, 2), 'note' => __('Last 7 days'), 'icon' => '＄', 'color' => '#27e498', 'light_color' => '#047857'],
            ['label' => __('Total Orders'), 'value' => number_format($weekOrders), 'note' => __('Last 7 days'), 'icon' => '🛒', 'color' => '#38bdf8', 'light_color' => '#0369a1'],
    ...
    <div class="grid gap-4 lg:grid-cols-3">
        <div class="rounded-2xl border border-slate-200 dark:border-[#29435e] bg-white dark:bg-[#10243c] p-5 lg:col-span-2">
            <div class="mb-4 flex flex-wrap items-center justify-between gap-2">
                ...
                <span class="rounded-lg border border-slate-200 dark:border-[#29435e] px-3 py-1.5 text-xs text-slate-600 dark:text-slate-300">{{ __('Last 7 Days') }}</span>
            </div>
            <svg viewBox="0 0 480 150" role="img" aria-label="{{ __('Sales over the last 7 days') }}" class="h-48 w-full" preserveAspectRatio="none">
                ...
                @foreach ($chartPoints as $point)
                    <circle cx="{{ $point['x'] }}" cy="{{ $point['y'] }}" r="4" fill="var(--dashboard-chart-accent)" stroke="var(--dashboard-chart-surface)" stroke-width="2"/>
                @endforeach
            </svg>
