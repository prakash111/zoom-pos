    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        <aside class="lg:col-span-4 space-y-5 lg:sticky lg:top-20 min-w-0">
            <div class="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200/80 dark:border-slate-800 p-5 xl:p-6 shadow-sm space-y-5">
                ...
                <button type="button" wire:click="save" wire:loading.attr="disabled" class="w-full py-3 px-4 rounded-2xl bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white font-bold text-xs shadow-lg shadow-blue-500/25 transition active:scale-[0.98] disabled:opacity-60"><span wire:loading.remove>⚡ {{ __('Save & Generate Proposal') }}</span><span wire:loading>{{ __('Saving...') }}</span></button>
            </div>
        </aside>
    </div>
