    ...
    <div class="lg:col-span-6 w-full">
        <div class="rounded-3xl bg-slate-900/90 backdrop-blur-xl border border-white/15 p-7 sm:p-9 shadow-2xl space-y-6">
            ...
            <div class="flex items-start justify-between border-b border-white/10 pb-4">
                <div>
                    <div class="flex items-center gap-2">
                        <h2 class="text-2xl sm:text-3xl font-black text-white tracking-tight">{{ __('Super Admin') }}</h2>
                    ...
                    <p class="text-xs text-slate-400 mt-1">{{ __('Platform management, tenant provisioning & system branding') }}</p>
                </div>
            ...
            <div class="space-y-4">
                ...
                <!-- Email Input -->
                <div class="space-y-1">
                    <label class="block text-xs font-bold text-slate-300 mb-1">{{ __('Admin Email Address *') }}</label>
                    <div class="relative flex items-center rounded-2xl border border-white/10 bg-white/5 overflow-hidden focus-within:ring-2 focus-within:ring-indigo-500 focus-within:border-indigo-500 transition">
                ...
                <div class="space-y-1" x-data="{ showPass: false }">
                    <div class="flex items-center justify-between">
                        <label class="block text-xs font-bold text-slate-300 mb-1">{{ __('Admin Password *') }}</label>
                    </div>
                    <div class="relative flex items-center rounded-2xl border border-white/10 bg-white/5 overflow-hidden focus-within:ring-2 focus-within:ring-indigo-500 focus-within:border-indigo-500 transition">
                        <div class="pl-3.5 pr-2 text-slate-400">
                            <div class="w-7 h-7 rounded-xl bg-white/10 flex items-center justify-center text-xs">
                ...
                <div class="flex items-center justify-between pt-1" x-data="{ remember: true }">
                    <span class="text-xs font-semibold text-slate-400">{{ __('Keep admin session active') }}</span>
            ...
            <div>
                <button type="button"
                        ...
                        class="w-full py-3.5 rounded-full bg-indigo-600 hover:bg-indigo-500 text-white font-black text-sm tracking-wide shadow-lg shadow-indigo-600/30 active:scale-95 transition flex items-center justify-center gap-2 cursor-pointer disabled:opacity-60">
            ...
            <div class="pt-4 border-t border-white/10 text-center">
                <a href="{{ route('tenant.login') }}" class="text-xs font-bold text-slate-400 hover:text-brand-lime transition inline-flex items-center gap-1">
        ...
        <div class="rounded-3xl bg-slate-900/60 backdrop-blur-xl border border-white/10 p-6 sm:p-8 space-y-6 shadow-2xl">
            <div class="flex items-center justify-between">
                <div class="flex items-center gap-2.5">
                    ...
                    <div>
                        <div class="text-sm font-black text-white">{{ __('Central Operations Node') }}</div>
            ...
            <div class="space-y-3 text-xs">
                <div class="p-3.5 rounded-2xl bg-white/5 border border-white/10 flex items-center justify-between">
                    <span class="text-slate-300">{{ __('Tenant Provisioning & Life-Cycle') }}</span>
                    <span class="text-emerald-400 font-bold">{{ __('Automated') }}</span>
                </div>
                <div class="p-3.5 rounded-2xl bg-white/5 border border-white/10 flex items-center justify-between">
                    <span class="text-slate-300">{{ __('White-label & Theming Engine') }}</span>
                    <span class="text-brand-lime font-bold">{{ __('Real-Time') }}</span>
                </div>
                <div class="p-3.5 rounded-2xl bg-white/5 border border-white/10 flex items-center justify-between">
                    <span class="text-slate-300">{{ __('Global Tax, Smtp & Audit Logs') }}</span>
