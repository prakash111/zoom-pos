<div class="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 items-start">
    ...
    <div class="lg:col-span-7 w-full">
        <div class="rounded-3xl bg-slate-900/90 backdrop-blur-xl border border-white/15 p-6 sm:p-8 shadow-2xl space-y-5">
            ...
            @if ($step === 2)
                ...
                <div class="flex items-start justify-between border-b border-white/10 pb-4">
                    <div>
                        <div class="flex items-center gap-2">
                            <h2 class="text-2xl sm:text-3xl font-black text-white tracking-tight">{{ __('Verify Your Email') }}</h2>
                    ...
                    <button type="button" wire:click="backToForm" class="text-xs font-black text-slate-400 hover:text-brand-lime transition shrink-0 text-right">
                ...
                <form wire:submit.prevent="verifyOtp" class="space-y-5 pt-2">
                    ...
                    <div class="pt-3 border-t border-white/10 text-center">
                        <button type="button" wire:click="backToForm" class="text-xs font-bold text-slate-400 hover:text-white transition inline-flex items-center gap-1 cursor-pointer">
            ...
            @else
                ...
                <div class="flex items-start justify-between border-b border-white/10 pb-4">
                    <div>
                        <div class="flex items-center gap-2">
                            <h2 class="text-2xl sm:text-3xl font-black text-white tracking-tight">{{ __('Create Your Store') }}</h2>
                            <span class="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full bg-brand-lime/20 text-brand-lime text-[10px] font-black border border-brand-lime/30">
                                {{ __('14-Day Free Trial') }}
                            </span>
                        </div>
                        <p class="text-xs text-slate-400 mt-1">{{ __('Deploy your POS & smart inventory environment in under 60 seconds') }}</p>
                    </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
                    <!-- Store Name Input -->
                    <div class="space-y-1">
                        <label class="block text-xs font-bold text-slate-300 mb-1">{{ __('Store / Business Name *') }}</label>
                        <div class="relative flex items-center rounded-2xl border border-white/10 bg-white/5 overflow-hidden focus-within:ring-2 focus-within:ring-brand-lime focus-within:border-brand-lime transition">
                            ...
                            <input type="text"
                                   ...
                                   class="w-full border-none bg-transparent text-xs sm:text-sm font-semibold text-white placeholder-slate-500 focus:ring-0 py-2.5 pr-3">
                        </div>
                    ...
                    <!-- Owner Name Input -->
                    <div class="space-y-1">
                        <label class="block text-xs font-bold text-slate-300 mb-1">{{ __('Admin / Owner Name *') }}</label>
                        <div class="relative flex items-center rounded-2xl border border-white/10 bg-white/5 overflow-hidden focus-within:ring-2 focus-within:ring-brand-lime focus-within:border-brand-lime transition">
                            <div class="pl-3.5 pr-2 text-slate-400">
                                <div class="w-7 h-7 rounded-xl bg-white/10 flex items-center justify-center text-xs">
                                    👤
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
                    <!-- Email Input -->
                    <div class="space-y-1">
                        <label class="block text-xs font-bold text-slate-300 mb-1">{{ __('Work Email Address *') }}</label>
                        <div class="relative flex items-center rounded-2xl border border-white/10 bg-white/5 overflow-hidden focus-within:ring-2 focus-within:ring-brand-lime focus-within:border-brand-lime transition">
                            <div class="pl-3.5 pr-2 text-slate-400">
                                <div class="w-7 h-7 rounded-xl bg-white/10 flex items-center justify-center text-xs">
                                    ✉️
                    ...
                    <div class="space-y-1" x-data="{ showPass: false }">
                        <label class="block text-xs font-bold text-slate-300 mb-1">{{ __('Create Password *') }}</label>
                        <div class="relative flex items-center rounded-2xl border border-white/10 bg-white/5 overflow-hidden focus-within:ring-2 focus-within:ring-brand-lime focus-within:border-brand-lime transition">
                            <div class="pl-3.5 pr-2 text-slate-400">
                                <div class="w-7 h-7 rounded-xl bg-white/10 flex items-center justify-center text-xs">
                                    🔒
                            ...
                            <input :type="showPass ? 'text' : 'password'"
                                   ...
                                   class="w-full border-none bg-transparent text-xs sm:text-sm font-semibold text-white placeholder-slate-500 focus:ring-0 py-2.5 pr-10">
                            <button type="button"
                                    x-on:click="showPass = !showPass"
                                    class="absolute right-3 text-slate-400 hover:text-white text-xs cursor-pointer">
                ...
                <div class="pt-1 space-y-2">
                    <button type="button"
                            ...
                            class="w-full py-2.5 px-4 rounded-2xl bg-white/5 hover:bg-white/10 border border-white/10 text-slate-300 text-xs font-bold flex items-center justify-between transition cursor-pointer">
                    ...
                    @if ($showDomainSettings)
                        <div class="p-4 rounded-2xl bg-white/5 border border-white/10 space-y-3">
                            ...
                            <div class="space-y-1">
                                ...
                                <div class="relative flex items-center rounded-xl border border-white/10 bg-slate-950 overflow-hidden focus-within:ring-2 focus-within:ring-brand-lime transition">
                                    <div class="pl-3 pr-1 text-slate-400 text-xs font-bold">🔗</div>
                                    <input type="text"
                                           wire:model.live.debounce.300ms="slug"
                                           placeholder="my-store"
                                           class="w-full border-none bg-transparent text-xs font-semibold text-white placeholder-slate-500 focus:ring-0 py-2 pr-1 font-mono">
                                    <div class="pr-3 pl-1 text-[11px] font-mono text-slate-400 shrink-0">.{{ $appHost }}</div>
                                </div>
                            ...
                            <div class="space-y-1">
                                ...
                                <div class="relative flex items-center rounded-xl border border-white/10 bg-slate-950 overflow-hidden focus-within:ring-2 focus-within:ring-brand-lime transition">
                                    ...
                                    <input type="text"
                                           ...
                                           class="w-full border-none bg-transparent text-xs font-semibold text-white placeholder-slate-500 focus:ring-0 py-2 pr-3 font-mono">
                                </div>
                    ...
                    <div class="space-y-1 pt-1">
                        ...
                        <div class="relative flex items-center rounded-2xl border border-brand-lime/40 bg-brand-lime/10 overflow-hidden focus-within:ring-2 focus-within:ring-brand-lime transition">
                            ...
                            <input type="text"
                                   ...
                                   class="w-full uppercase tracking-wider font-mono border-none bg-transparent text-xs sm:text-sm font-black text-white placeholder-brand-lime/50 focus:ring-0 py-2.5 pr-3">
                        </div>
            ...
            <div class="pt-4 border-t border-white/10 text-center">
                <span class="text-xs text-slate-400">{{ __('Already have an active store account?') }}</span>
    ...
    <div class="lg:col-span-5 space-y-5">
        ...
        <!-- Guarantee Card -->
        <div class="rounded-3xl bg-slate-900/60 backdrop-blur-xl border border-white/10 p-6 sm:p-7 space-y-5 shadow-2xl">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h3 class="text-base font-black text-white">{{ __('Instant Cloud Provisioning') }}</h3>
                    <p class="text-xs text-slate-400">{{ __('Ready in <60s with full hardware support') }}</p>
                </div>
            </div>
            ...
            <div class="p-3.5 rounded-2xl bg-white/5 border border-white/10 flex items-center justify-between text-[11px] text-slate-300">
        ...
        <div class="rounded-3xl bg-slate-900/40 backdrop-blur-xl border border-white/10 p-5 space-y-2.5 text-xs">
