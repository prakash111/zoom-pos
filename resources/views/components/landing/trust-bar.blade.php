...
@php
    $hardware = [
        ['label' => 'Barcode Scanners', 'icon' => 'barcode', 'tag' => 'Instant Scan'],
        ['label' => 'Thermal Receipt Printers', 'icon' => 'printer', 'tag' => '80mm / 58mm'],
        ['label' => 'Card Readers & Terminals', 'icon' => 'card', 'tag' => 'EMV & NFC'],
        ['label' => 'Smart Cash Drawers', 'icon' => 'drawer', 'tag' => 'Auto Kick'],
        ['label' => 'Kitchen Display Screens', 'icon' => 'display', 'tag' => 'Live KDS'],
    ];
@endphp

<div class="max-w-6xl mx-auto px-4 sm:px-6 -mt-3 sm:-mt-5 relative z-30">
    <div class="rounded-3xl bg-slate-900/90 dark:bg-slate-950/90 backdrop-blur-2xl border border-white/15 dark:border-slate-800 shadow-2xl px-6 sm:px-10 py-6 sm:py-7">
        <p class="text-center text-[11px] font-black uppercase tracking-widest text-slate-400 mb-6 flex items-center justify-center gap-2">
            <span class="w-1.5 h-1.5 rounded-full bg-brand-lime animate-pulse"></span>
            <span>{{ __('Works out of the box with your existing retail & dining hardware') }}</span>
        </p>
        <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4 sm:gap-6">
            @foreach ($hardware as $item)
                <div class="flex flex-col items-center justify-center text-center p-3.5 rounded-2xl bg-white/5 hover:bg-white/10 border border-white/5 hover:border-brand-lime/30 transition-all group">
                    <div class="w-10 h-10 rounded-xl bg-white/10 text-brand-lime flex items-center justify-center mb-2.5 group-hover:scale-110 transition-transform">
                    ...
                    <span class="text-xs font-black text-white leading-tight">{{ __($item['label']) }}</span>
                    <span class="text-[10px] text-slate-400 font-medium mt-1">{{ __($item['tag']) }}</span>
                </div>
            @endforeach
        </div>
    </div>
