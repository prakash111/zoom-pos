...
<section id="features" class="landing-sec-features py-20 sm:py-28 scroll-mt-20 transition-colors duration-300">
    ...
    <div class="text-center mb-12 sm:mb-16">
        ...
        <h2 class="text-3xl sm:text-5xl font-black tracking-tight text-white">{{ $branding->getSectionTitle('features', __('Everything Your Business Needs to Maximize Internet & In-Store Sales')) }}</h2>
        <p class="mt-3 text-sm sm:text-base text-slate-400 max-w-2xl mx-auto">{{ $branding->getSectionSubtitle('features', __('From online catalogs and WhatsApp orders to barcode checkout, multi-store stock, and automated tax invoicing — one synchronized cloud platform.')) }}</p>
    </div>
                    ...
                    <div>
                        ...
                        <ul class="space-y-3.5">
                            @foreach ($f['points'] as $point)
                                ...
                                @endphp
                                <li class="flex items-start gap-3 text-sm leading-relaxed {{ $isBenefit ? 'text-brand-lime font-bold p-3 rounded-xl bg-brand-lime/10 border border-brand-lime/20 mt-2' : 'text-slate-300' }}">
                                    <div class="w-5 h-5 rounded-full {{ $isBenefit ? 'bg-brand-lime/20 text-brand-lime' : 'bg-emerald-500/20 text-emerald-400' }} flex items-center justify-center text-xs font-black shrink-0 mt-0.5">✓</div>
                                    <span>{!! $point !!}</span>
                                </li>
                            @endforeach
                        </ul>
