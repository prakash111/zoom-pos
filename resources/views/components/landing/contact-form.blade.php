...
<div class="max-w-xl mx-auto"
    ...
    <form x-ref="form" x-show="!sent" method="POST" action="{{ route('contact.store') }}"
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
                <label for="cf_name" class="block text-xs font-bold text-slate-200 mb-1">{{ __('Name *') }}</label>
                <input id="cf_name" type="text" name="name" required minlength="2" value="{{ old('name') }}"
                       class="w-full rounded-xl border-white/15 bg-white/5 text-white text-sm focus:border-brand-lime focus:ring-brand-lime transition-colors">
                @error('name') <p class="text-rose-400 text-xs mt-1">{{ $message }}</p> @enderror
            </div>
            <div>
                <label for="cf_email" class="block text-xs font-bold text-slate-200 mb-1">{{ __('Business Email *') }}</label>
            ...
            <div>
                <label for="cf_store_type" class="block text-xs font-bold text-slate-200 mb-1">{{ __('Store Type') }}</label>
                <select id="cf_store_type" name="store_type"
                        class="w-full rounded-xl border-white/15 bg-white/5 text-white text-sm focus:border-brand-lime focus:ring-brand-lime">
                    <option value="" class="bg-slate-900">{{ __('Select your business type…') }}</option>
                    @foreach (['Retail Store', 'Supermarket / Grocery', 'Restaurant / Cafe', 'Bar / Lounge', 'Service Business', 'Other'] as $type)
                        <option value="{{ $type }}" class="bg-slate-900" @selected(old('store_type') === $type)>{{ __($type) }}</option>
                    @endforeach
                </select>
            </div>
            <div>
                <label for="cf_phone" class="block text-xs font-bold text-slate-200 mb-1">{{ __('Phone') }}</label>
        ...
        <div>
            <label for="cf_message" class="block text-xs font-bold text-slate-200 mb-1">{{ __('Message *') }}</label>
            <textarea id="cf_message" name="message" rows="4" required minlength="10"
                      ...
                      class="w-full rounded-xl border-white/15 bg-white/5 text-white text-sm focus:border-brand-lime focus:ring-brand-lime transition-colors">{{ old('message') }}</textarea>
