...
class DispatchScheduledNotifications extends Command
{
    ...
    {
        $this->dispatchOrderAlarms($push);
        $this->dispatchInvoiceReminders($push);

        return self::SUCCESS;
    }

    private function dispatchOrderAlarms(FirebasePushService $push): void
    {
        KitchenTicket::withoutGlobalScope('company')
            ...
            ->chunkById(100, function ($tickets) use ($push) {
    ...
    private function dispatchInvoiceReminders(FirebasePushService $push): void
    {
        Sale::withoutGlobalScope('company')
            ...
            ->chunkById(100, function ($sales) use ($push) {
