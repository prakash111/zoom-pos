...
class NotificationController extends Controller
{
    ...
    public function feed(Request $request): JsonResponse
    {
        ...
        $webRequest = $request->is('tenant/*');
        $clearEndpoint = $webRequest
            ? '/tenant/notifications/clear-all'
            : '/api/v1/tenant/notifications/clear-all';

        // A freshly reset demo workspace should still demonstrate the sheet
        // even before its first sale/KOT/follow-up is created.
        if ($alerts->isEmpty() && Company::withoutGlobalScopes()->find($tenantId)?->is_demo) {
            $alerts->push([
                'type' => 'notification_item',
                'id' => 'demo_workspace_ready',
                'category' => 'system',
                'icon' => 'check_circle',
                'icon_color' => '#10B981',
                'title' => 'Workspace Ready',
                'subtitle' => 'All enabled modules and extensions are ready to explore.',
        ...
        if ($alerts->isEmpty()) {
            ...
            // Sheet header with the clear-all action.
            $components[] = [
                'type' => 'row',
                ...
                'children' => [
                    ['type' => 'text', 'value' => 'Alerts & Reminders ('.$alerts->count().')', 'style' => 'title_medium', 'weight' => 'bold'],
                    ['type' => 'button_danger', 'label' => 'Clear All', 'icon' => 'delete_sweep', 'dense' => true,
                        'confirm_message' => 'Are you sure you want to dismiss all alerts and reminders?',
                        'action' => ['type' => 'SUBMIT_FORM', 'endpoint' => $clearEndpoint, 'method' => 'POST', 'reload' => true]],
                ],
            ];
            ...
            foreach ($alerts as $alert) {
                ...
                $dismissEndpoint = $webRequest
                    ? "/tenant/notifications/{$alertCategory}/{$alertId}/dismiss"
                    : "/api/v1/tenant/notifications/{$alertCategory}/{$alertId}/dismiss";
                $components[] = [
                    'type' => 'column', 'margin' => ['bottom' => 6],
                    'children' => [$alert, [
                        'type' => 'row', 'main_axis_alignment' => 'end', 'padding' => ['top' => 2, 'bottom' => 4, 'right' => 4],
                        'children' => [[
                            'type' => 'button_outlined', 'label' => 'Dismiss', 'icon' => 'close', 'dense' => true,
                            'color' => '#EF4444', 'border_color' => '#FCA5A5',
                            'action' => ['type' => 'SUBMIT_FORM', 'endpoint' => $dismissEndpoint, 'method' => 'POST', 'data' => ['type' => $alertCategory, 'id' => $alertId], 'reload' => true],
                        ]],
                    ]],
                ];
            }
